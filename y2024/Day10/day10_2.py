...
def get_my_answer():
    def check_path(height, position, beginning, this_trail):
        if height == 9:
            trailheads[beginning].add(this_trail)
            return
        for dirx, diry in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
            ...
            if (0 <= new_position[0] <= max_x) and (0 <= new_position[1] <= max_y) and (int(data[new_position[1]][new_position[0]]) == (height + 1)):
                check_path(height + 1, new_position, beginning, tuple(list(this_trail) + [new_position[0], new_position[1]]))
    ...
    for start_position in start_positions:
        check_path(0, start_position, start_position, (start_position[0], start_position[1]))
