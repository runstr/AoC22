...
def calculate_bounding_box(start, end, max_x, max_y):
    ...
    :return: List of all cordinates of bounding elemetns
    """
    bounding_indexes = []
    ...
    for line in data:
        ...
        for x, step in enumerate(line):
            ...
            if step == ".":
                continue
            if step == "*":
                symbol_indexes.append(((x, y), step))
...
def get_my_answer():
    ...
    for num, start, end in numbers:
        ...
        all_numbers.append((num, bounding_box))
    all_gear_numbers = []
    for index, symbol in symbol_indexes:
        gear_numbers = []
        for num, bounding_box in all_numbers:
            if index in bounding_box:
                gear_numbers.append(num)
        all_gear_numbers.append(gear_numbers)


    my_answer = 0
    for gear_numbers in all_gear_numbers:
        if len(gear_numbers) == 2:
            my_answer += (gear_numbers[0]*gear_numbers[1])
