...
def expand_and_map_input(data):
    ...
    for y, line in enumerate(data):
        line = "."*12+line+"."*12
        data[y] = line
    data = ["." * len(data[0]) for _ in range(6)] + data + ["." * len(data[0]) for _ in range(6)]
    # Map data
    for y, line in enumerate(data):
...
def get_my_answer():
    ...
    minx, maxx, miny, maxy = boundin_box(mapping)
    open_areas = count_open(mapping, minx, maxx, miny, maxy)
    return open_areas
