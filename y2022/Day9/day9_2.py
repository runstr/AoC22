...
def move_tail_closer(head_position, tail_position, all_tail_positions, tail):
...
def check_knot_positions(head_position, knot_positions, all_tail_positions):
    tail=False
    for i in range(0, len(knot_positions)):
        if i == 0:
            if abs(head_position[0]-knot_positions[i][0])>1 or abs(head_position[1]-knot_positions[i][1])>1:
                knot_positions[i] = move_tail_closer(head_position, knot_positions[i], all_tail_positions, tail=tail)
        else:
            if i==8:
                tail = True
            if abs(knot_positions[i-1][0]-knot_positions[i][0])>1 or abs(knot_positions[i-1][1]-knot_positions[i][1])>1:
                knot_positions[i] = move_tail_closer(knot_positions[i-1], knot_positions[i], all_tail_positions, tail=tail)
...
def move_head(movement, head_position, tail_position, all_tail_positions):
    ...
    for i in range(int(steps)):
        ...
        if direction == "L":
            head_position = (head_position[0]-1, head_position[1])
        tail_position = check_knot_positions(head_position, tail_position, all_tail_positions)
...
def get_my_answer():
    ...
    for movement in data:
        head_position, knot_positions = move_head(movement, head_position, knot_positions, all_knot_positions)
    return len(all_knot_positions)
