...
def get_my_answer():
    ...
    print("found paths")
    sorted_path_pressure_keys = sorted(total_paths.items(), key=operator.itemgetter(1), reverse=True)
    print("sorted paths")
    max_value = max(list(total_paths.values()))
    valid_paths = []
    for key1, value1 in sorted_path_pressure_keys:
        for key2, value2 in sorted_path_pressure_keys:
            if key1 == key2:
                continue
            if value2+value1 <= max_value:
                continue
            same_caves = False
            for cave1 in key1.split("-"):
                if cave1 in key2.split("-") and cave1 !="AA":
                    same_caves = True
                    break
            if not same_caves:
                max_value=max(max_value, value2+value1)
                valid_paths.append(value2+value1)
                print(max_value)
